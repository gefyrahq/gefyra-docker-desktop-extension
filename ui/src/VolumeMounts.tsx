        ...
        if (mode == "host") {
            ...
            if (!result.canceled) {
              ...
              if (directory !== undefined) {
                dispatch(setVolumeMount({volumeMount: {host: directory, container: store.getState().gefyra.volumeMounts[index].container}, index: index}))
              }
            }
        } else {
                dispatch(setVolumeMount({volumeMount: {host: store.getState().gefyra.volumeMounts[index].host, container: e.target.value}, index: index}))
            }
    };
                ...
                {volumeMounts.map((v, index) => 
                    ( v ?
                        <Grid container spacing={4} key={index}>
                                ...
                                <Grid item xs={5}>
                                    <InputLabel sx={{ mb: 1 }} id="variable-label">Container Path</InputLabel>
                                    <TextField id="container" variant="outlined" fullWidth InputProps={{ readOnly: false }} onChange={(e) => handleHostVolumeMountChange(e, index, 'container')} value={store.getState().gefyra.volumeMounts[index].container}/>
                                </Grid>
                                <Grid item xs={2}>
                                    <Button variant="contained" color="error" sx={{mt: 5}} onClick={() => dispatch(removeVolumeMount(index))}>X</Button>
                                </Grid>
                        </Grid>
                    : '')
                )}

...
export const checkCargoReady = async (gefyraClient: Gefyra, times: number = 1, resolver = null, rejecter = null) => {
    /*
     * Check whether Stowaway is ready and available.
     * `times` determines how often the check is repeated before failing.
     */
    ...
    return new Promise((resolve, reject) => {
            gefyraClient.exec(statusRequest).then(async (res) => {
                ...
                if (client.cargo) {
                    ...
                    setTimeout(() => {
                        checkCargoReady(gefyraClient, times - 1, resolver ? resolver : resolve, reject).catch(err => reject()).then(res => resolve(true))
                    }, 1000)
                }
            })
        })
